...
from .util import sigmoid, ReLU, tanh, linear
...
class NNController(Controller):
    def __init__(self, hidden_layers=[5], activation_funcs=[ReLU, linear], weight_range=(-.1, .1), bias_range=(-.1, .1)) -> None:
        super().__init__()
    ...
    def gen_params(self):
        ...
        for outputs in layers:
            weights = np.random.uniform(
                self.weight_range[0], self.weight_range[1], (inputs, outputs))
            biases = np.random.uniform(
                self.bias_range[0], self.bias_range[1], (1, outputs))
            inputs = outputs
